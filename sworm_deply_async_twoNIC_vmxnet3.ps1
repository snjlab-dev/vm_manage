...
if ($from -eq "template"){
    foreach($col in $list) {
        ...
        if([boolean]::Parse($apply) -eq $true){
            New-VM -Name $name -Template $template -datastore $ds -vmhost $esxi -DiskStorageFormat Thin -RunAsync
        }
    }
}else{
     foreach($col in $list) {
            ...
            if([boolean]::Parse($apply) -eq $true){
                ...
                New-VM -Name $name -VM $template -datastore $ds -vmhost $esxi -DiskStorageFormat Thin -RunAsync
            }
        }
}
        ...
        if([boolean]::Parse($apply) -eq $true){
            while($true){
                ...
                If ($Exists){
                    ...
                    $win_nic1 = '"Ethernet0 2"'
                    $win_nic2 = '"Ethernet1 2"'
